...
const Navbar = () => {
  ...
  const { selectedTab, setSelectedTab, setcurrentContent } =
    useContentControlContext();
        ...
        <button className="navHeaderButton" onClick={() => handleTabSwitch(0)}>
          ...
          >
            <hr
              ...
              style={{
                background: buttonGradientSnippet(colorNumber),
                border: "none",
                padding: ".5px",
      ...
      <div className="navbarBody">
        <div className={selectedTab === 0 ? "active" : ""}>
          <div
            ...
            style={selectedTab === 0 ? mountedStyle : unmountedStyle}
          >
            {experience.map((item) => {
              return (
                <MyButton
                  text={item.companyName}
                  onClick={() => handleButtonClick(item.companyName)}
                />
              );
            })}
          </div>
        ...
        <div className={selectedTab === 1 ? "active" : ""}>
          <div
            className="tabOptions"
            style={selectedTab === 1 ? mountedStyle : unmountedStyle}
          >
            {projects.map((item) => {
              return (
                <MyButton
